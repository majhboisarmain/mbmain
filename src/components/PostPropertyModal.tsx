...
import { X, CheckCircle, Camera, Coins, FileText, Check, Upload, Trash2, Star, MessageSquare, Video, ShieldCheck, MapPin } from 'lucide-react';
...
export const PROPERTY_REGIONS = ['Boisar', 'Palghar', 'Umroli'] as const;
...
export default function PostPropertyModal({ isOpen, onClose, onAddProperty }: PostPropertyModalProps) {
  ...
  const [selectedArea, setSelectedArea] = useState('Boisar West');
  ...
  const handleFormSubmit = async () => {
    ...
    const cleanAddress = addressLocality.trim();
    const effectiveLocality = selectedArea
      ? (cleanAddress ? `${selectedArea}, ${cleanAddress}` : selectedArea)
      : (cleanAddress || 'Boisar');
    const projectOrLocality = projectName.trim() ? `${projectName.trim()}, ${effectiveLocality}` : effectiveLocality;
    ...
    try {
      const res = await fetch('/api/properties', {
        ...
        body: JSON.stringify({
          ...
          addressLocality: effectiveLocality,
                    ...
                    <div className="flex flex-wrap gap-2">
                      {PROPERTY_REGIONS.map((reg) => (
                        <button
                          ...
                          onClick={() => {
                            setCityName(reg);
                            if (reg === 'Palghar') setSelectedArea('Palghar');
                            else if (reg === 'Umroli') setSelectedArea('Umroli');
                            else if (selectedArea === 'Palghar' || selectedArea === 'Umroli') setSelectedArea('Boisar West');
                          }}
                          className={`px-3.5 py-1.5 rounded-xl text-xs font-black transition-all border text-center cursor-pointer ${
                            cityName === reg
